...
- [5. Output Schema](#5-output-schema)
  ...
  - [5.11. Relationship Annotation Fields](#511-relationship-annotation-fields)
  - [5.12. Dropped and Restructured Fields](#512-dropped-and-restructured-fields)
  - [5.13. Schema Validation and Enforcement](#513-schema-validation-and-enforcement)
  - [5.14. Backward Compatibility Considerations](#514-backward-compatibility-considerations)
- [6. Core Operations](#6-core-operations)
  ...
  - [6.7. Relationship Classification](#67-relationship-classification)
  ...
  - [6.11. Rollback Operations](#611-rollback-operations)
- [7. Configuration](#7-configuration)
  ...
  - [7.5. Sidecar Rule Engine](#75-sidecar-rule-engine)
...
- [8. CLI Interface](#8-cli-interface)
  ...
  - [8.4. Metadata and Relationship Processing Options](#84-metadata-and-relationship-processing-options)
...
- [16. Security and Safety](#16-security-and-safety)
  ...
  - [16.4. Large File and Deep Recursion Handling](#164-large-file-and-deep-recursion-handling)
...
This document is the authoritative technical specification for `shruggie-indexer`, a cross-platform file and directory indexing tool that produces structured JSON output with hash-based identities, filesystem metadata, EXIF data, and relationship annotations between files. It serves as the single source of truth for the tool's behavioral contract, architecture, and release engineering, and as the primary reference for maintaining and extending the project.
...
- **Core indexing engine.** Filesystem traversal, file and directory identity generation (hashing), symlink detection, timestamp extraction and conversion, EXIF/embedded metadata extraction via `exiftool`, relationship classification via a rule-based sidecar detection engine, index entry construction, and JSON output serialization. The engine is a pure library with no presentation-layer dependencies. The CLI and GUI are thin frontends that delegate all indexing work to this layer.

- **Dependency architecture.** The tool relies on `exiftool` as its sole external binary dependency for embedded metadata extraction. All other data processing (hashing, timestamp derivation, JSON parsing, relationship classification) uses Python standard library modules or a small set of declared third-party packages. See [§12](#12-external-dependencies) for the complete dependency inventory.
...
- **Output schema (v4).** The complete JSON output schema for index entries as defined by the v4 schema. The v4 schema introduces a `relationships[]` array on IndexEntry for annotating believed associations between files, removes sidecar-origin content embedding from `MetadataEntry`, and adopts permanent `_idx.json`/`_idxd.json` output suffixes decoupled from the schema version number. ExifTool metadata remains in the `metadata[]` array with `origin: "exiftool"`. A `schema_version` discriminator field is present with the value `4`.

  > **Historical note:** The v1 schema (`MakeIndex_OutputSchema.json`) documents the original MakeIndex output structure and remains available as a porting reference. The v2 schema was the initial Python implementation target, consolidating related fields into logical sub-objects. The v4 schema (the current target) eliminates the sidecar ingest/embed pipeline and replaces it with relationship annotation. See the sidecar architecture pivot document ([§1.5](#15-reference-documents)) for the full rationale.

- **Relationship classification system.** The rule-based engine that examines the file inventory after the traversal pass and annotates IndexEntry objects with directional relationship data. This system uses TOML-configured rules with pattern matching (`{stem}`, `*` tokens), predicates (`requires_sibling`, `requires_sibling_any`, `excludes_sibling`), confidence scoring, and provenance tracking (`builtin`, `user`, `pack:<name>`). Rules are shipped as a built-in library, user-configurable, and extensible via community rule packs.

- **CLI interface.** The command-line interface exposing all indexing operations, including target selection, output mode control, metadata processing options, relationship detection control, rename operations, ID type selection, and verbosity/logging configuration.
...
- **Configuration system.** The externalized configuration architecture, including default values, file format, override/merge behavior, and the sidecar rule engine configuration (`[sidecar_rules.*]` TOML tables).

- **Logging and diagnostics.** The structured logging system, including logger hierarchy, log levels, session identifiers, and output destinations.
...
- **Security and safety.** Symlink traversal safety, path validation, temporary file handling, and large file/deep recursion handling.
...
- **The `Encoding` output field.** The v1 output schema included an `Encoding` key containing detailed file encoding properties derived from BOM byte inspection. This field is intentionally absent from the v4 schema. See [§5.12](#512-dropped-and-restructured-fields) for the full rationale.

- **The v1-to-v4 migration utility.** A migration script for converting existing v1 index assets to the v4 schema format is a planned deliverable but is deferred to post-MVP. It is not part of the current release.

- **The original PowerShell source code.** The original `main.ps1` is closed-source and is not included in the `shruggie-indexer` repository. The original source code for the `MakeIndex` function SHALL NOT be included in the repository in any form. All behavioral knowledge required for implementation and maintenance is captured in this specification and the reference documents listed in [§1.5](#15-reference-documents).

- **Sidecar content ingestion.** The v4 architecture removes all sidecar content parsing, embedding, and reconstruction. The indexer does not read, store, or re-serialize sidecar file content. Sidecar files are indexed as first-class entries with their own identity, and the relationship classifier annotates the association between sidecar-like files and their parents. See the sidecar architecture pivot document ([§1.5](#15-reference-documents)) for the complete rationale.
...
Updates to this document SHOULD be committed alongside the code changes they describe. Significant architectural changes MUST be reflected in this specification before or concurrent with the implementation.
...
- `§N.N` denotes a cross-reference to another section of this specification (e.g., [§5.2](#52-reusable-type-definitions) refers to section 5.2, "Reusable Type Definitions").
...
| **v2 schema** | The initial Python output format defined by `shruggie-indexer-v2.schema.json`. Retained for backward compatibility in rollback operations. Superseded by v4. |
| **v4 schema** | The current output format. Introduces `relationships[]` on IndexEntry, removes sidecar-origin MetadataEntry fields, and adopts `_idx.json`/`_idxd.json` as permanent output suffixes. This is the target schema for all new implementation work. |
| **Index entry** | A single structured data object representing a file or directory in the v4 output schema. Defined in [§5](#5-output-schema). |
| **Sidecar file** | An external metadata file that lives alongside the file it describes, identified by rule-based pattern matching. In the v4 architecture, sidecar files are indexed as first-class entries with relationship annotations rather than being ingested into their parent's metadata. |
| **Relationship annotation** | A directional annotation on an IndexEntry indicating a believed association with another indexed file. Produced by the relationship classifier based on sidecar detection rules. See [§5.11](#511-relationship-annotation-fields). |
...
| **HashSet** | A schema object containing hash digests for a given input. Always includes `md5` and `sha256`; optionally includes `sha512`. |
| **NameObject** | A schema object pairing a `text` string with its `hashes` (a HashSet). |
| **TimestampPair** | A schema object containing both an `iso` (ISO 8601 string) and `unix` (milliseconds since epoch) representation of a single timestamp. |
...
| **StorageName (`storage_name`)** | The deterministic filename derived from an item's `id` and extension. Used when the rename operation is active. |
| **In-place write** | Writing individual `_idx.json` or `_idxd.json` sidecar files alongside each processed item, as opposed to writing a single aggregate output file. |
| **Rule engine** | The TOML-configured classification system that matches file names against sidecar rules and produces relationship annotations. See [§7.5](#75-sidecar-rule-engine). |
| **Community rule pack** | A TOML file containing a set of sidecar rules that can be installed and loaded by the rule engine. See [§7.5](#75-sidecar-rule-engine). |
| **Confidence level** | An integer code (1, 2, or 3) on a relationship annotation indicating how strongly the classifier believes the relationship exists. See [§5.11](#511-relationship-annotation-fields). |
...
|----------|------|-------------|
| Implementation Plan | [`.archive/20260219-002-shruggie-indexer-plan.md`](.archive/20260219-002-shruggie-indexer-plan.md) | Sprint-based implementation plan that guided initial construction through Sprints 1.1–3.3 and the v0.1.0 release. All sprints complete. Archived for historical reference only. |
| Sidecar Architecture Pivot | [`.archive/20260401-001-sidecar-architecture-pivot.md`](.archive/20260401-001-sidecar-architecture-pivot.md) | Defines the fundamental architectural change from embedded sidecar processing to relationship annotation. Ratified 2026-03-24, new edition 2026-04-01. Supersedes `20260321-005-sidecar-architecture-pivot.md`. This document is the authoritative source for the v4 schema design, the relationship annotation model, the sidecar rule engine, and the implementation sequencing for the pivot. All architectural decisions in this specification that pertain to relationship classification, the v4 schema, or the removal of sidecar content processing derive from this pivot document. |

#### Output Schema

| Document | Location | Description |
|----------|----------|-------------|
| v4 Schema (canonical) | [schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json](https://schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json) | The canonical JSON Schema definition for the v4 index entry output format. This is the target schema for all implementation work. Adds `relationships[]` to IndexEntry. Removes sidecar-origin MetadataEntry fields. Adopts `_idx.json`/`_idxd.json` as permanent output suffixes. |
| v4 Schema (local copy) | [`./docs/schema/shruggie-indexer-v4.schema.json`](./docs/schema/shruggie-indexer-v4.schema.json) | Local copy of the canonical v4 schema, committed to the repository. Used by the documentation site ([§3.7](#37-documentation-site)) and as a local validation reference. This file MUST be kept in sync with the canonical hosted version. |
| v2 Schema (legacy reference) | [`./docs/schema/shruggie-indexer-v2.schema.json`](./docs/schema/shruggie-indexer-v2.schema.json) | The v2 JSON Schema definition. Retained for backward compatibility support in the rollback engine (v2/v3 sidecar files can still be rolled back via the legacy path). The tool does not produce v2 output. |
| v1 Schema (porting reference) | [`./docs/porting-reference/MakeIndex_OutputSchema.json`](./docs/porting-reference/MakeIndex_OutputSchema.json) | The JSON Schema definition for the original MakeIndex v1 output format. Retained as a porting reference. shruggie-indexer does NOT target this schema. |
...
| Document | Path | Description |
|----------|------|-------------|
| MetadataFileParser Object | [`./docs/porting-reference/MakeIndex(MetadataFileParser).ps1`](./docs/porting-reference/MakeIndex%28MetadataFileParser%29.ps1) | Isolated PowerShell script containing the complete `$global:MetadataFileParser` object definition. This is the historical source for sidecar metadata file discovery patterns, per-type behavioral attributes, exiftool exclusion lists, extension group classifications, and indexer include/exclude patterns. In the v4 architecture, the sidecar type identification patterns have been superseded by the TOML-based rule engine ([§7.5](#75-sidecar-rule-engine)), but the regex patterns remain a valuable reference for authoring built-in rules. |

#### Dependency Catalogs

Each dependency catalog documents a single function from the original pslib library that `MakeIndex` depends on.

| Document | Path | Original Function |
|----------|------|-------------------|
| Base64DecodeString | [`./docs/porting-reference/Base64DecodeString_DependencyCatalog.md`](./docs/porting-reference/Base64DecodeString_DependencyCatalog.md) | Eliminated. |
| Date2UnixTime | [`./docs/porting-reference/Date2UnixTime_DependencyCatalog.md`](./docs/porting-reference/Date2UnixTime_DependencyCatalog.md) | Eliminated. |
| DirectoryId | [`./docs/porting-reference/DirectoryId_DependencyCatalog.md`](./docs/porting-reference/DirectoryId_DependencyCatalog.md) | Two-layer directory identity scheme. |
| FileId | [`./docs/porting-reference/FileId_DependencyCatalog.md`](./docs/porting-reference/FileId_DependencyCatalog.md) | File content/name hashing. |
| MakeIndex | [`./docs/porting-reference/MakeIndex_DependencyCatalog.md`](./docs/porting-reference/MakeIndex_DependencyCatalog.md) | Top-level orchestration. |
| MetaFileRead | [`./docs/porting-reference/MetaFileRead_DependencyCatalog.md`](./docs/porting-reference/MetaFileRead_DependencyCatalog.md) | Sidecar reading (historical reference; the v4 architecture does not read sidecar content). |
| TempOpen | [`./docs/porting-reference/TempOpen_DependencyCatalog.md`](./docs/porting-reference/TempOpen_DependencyCatalog.md) | Eliminated. |
| TempClose | [`./docs/porting-reference/TempClose_DependencyCatalog.md`](./docs/porting-reference/TempClose_DependencyCatalog.md) | Eliminated. |
| Vbs | [`./docs/porting-reference/Vbs_DependencyCatalog.md`](./docs/porting-reference/Vbs_DependencyCatalog.md) | Replaced by Python `logging`. |

#### External References

| Document | URL | Description |
|----------|-----|-------------|
| RFC 2119 | [https://www.rfc-editor.org/rfc/rfc2119](https://www.rfc-editor.org/rfc/rfc2119) | Requirement level keywords used throughout this specification. |
| JSON Schema Draft-07 | [https://json-schema.org/draft-07/schema#](https://json-schema.org/draft-07/schema#) | The JSON Schema dialect used by all output schemas. |
| ExifTool | [https://exiftool.org/](https://exiftool.org/) | The external binary dependency for embedded metadata extraction. |
| shruggie-feedtools | [https://github.com/shruggietech/shruggie-feedtools](https://github.com/shruggietech/shruggie-feedtools) | Sibling ShruggieTech project whose specification, repository layout, and GUI design serve as the primary architectural and visual reference for `shruggie-indexer`. |
| ShruggieTech Unified Product Vision | (internal, v1.0.0, 2026-03-30) | Establishes the three-layer architecture (open-source foundation, internal infrastructure, commercial products) and identifies the sidecar architecture pivot and v4 schema as Phase 1 deliverables. |
| Metadexer Technical Specification | (DRAFT, 2026-03-19) | Defines the vault, catalog, and sync modules that consume IndexEntry output. Currently references v2; requires updating to v4. |
...
| Repository | [shruggietech/shruggie-indexer](https://github.com/shruggietech/shruggie-indexer) |
| Organization | Shruggie LLC (DBA ShruggieTech) |
| License | Apache 2.0 ([full text](https://www.apache.org/licenses/LICENSE-2.0)) |
...
`shruggie-indexer` is a standalone project within the ShruggieTech tool family. It shares no code with `shruggie-feedtools` or any other ShruggieTech project at runtime, but it follows the same repository conventions, packaging patterns, build tooling, and GUI design language established by `shruggie-feedtools` (see [§1.5](#15-reference-documents), External References). Where this specification does not explicitly define a convention, the `shruggie-feedtools` repository serves as the normative reference for project scaffolding.
...
The tool carries forward the original's core behavioral contract: given a target path (file or directory), produce a JSON index entry containing hash-based identities, filesystem metadata, timestamps, and embedded EXIF data, all structured according to a defined output schema. The v4 architecture adds relationship annotation (identifying associations between files on disk) while removing the sidecar content ingestion pipeline that the original and the v2 implementation shared. Everything else (the language, the architecture, the output schema version, the dependency set, the configuration model, and the platform scope) is designed for the Python ecosystem and cross-platform execution.
...
> **Historical note:** The original `MakeIndex` is one function among hundreds in `main.ps1`, a monolithic 17,000+ line PowerShell script that serves as a general-purpose utility library. `MakeIndex` itself spans approximately 1,500 lines with 20+ nested sub-functions. It depends on eight additional top-level pslib functions and two external binaries. The combined dependency tree encompasses roughly 60 discrete code units.

### 2.3. Design Goals and Non-Goals

#### Design Goals

**G1 — Behavioral fidelity.** For any input path, the tool MUST produce a v4 index entry whose semantic content is correct and complete: the same file always produces the same hash-based identity, the same timestamp values (within platform precision limits), the same embedded metadata extraction results, and the same relationship classification outcomes. "Semantic equivalence" means deterministic, reproducible output. It does NOT mean byte-identical JSON across runs; timestamp access times and output formatting may vary.

> **Historical note (G1):** Behavioral fidelity was originally defined relative to the PowerShell `MakeIndex` function's v1 output. The v4 architecture extends fidelity to include relationship annotations: the same filesystem layout MUST produce the same set of relationships with the same confidence levels and predicate outcomes.

**G2 — Cross-platform portability.** The tool MUST run on Windows, Linux, and macOS without platform-specific code branches in the core indexing engine. Platform-specific behavior (such as creation time availability or symlink semantics) is handled through documented abstractions with explicit fallback strategies. The only external binary dependency, `exiftool`, is itself cross-platform.
...
**G4 — Externalized, user-modifiable configuration.** All behavioral parameters (sidecar detection rules, exiftool exclusion lists, filesystem exclusion filters, extension validation rules) are externalized into a typed configuration system with sensible defaults, a documented TOML configuration file format, and a merge/override mechanism for user customization. A user SHOULD be able to add a new sidecar detection rule, extend the exiftool exclusion list, or modify the filesystem exclusion filters without editing source code.

**G5 — Dependency minimization.** The tool declares a small, deliberately chosen set of required runtime dependencies: `click` (CLI parsing), `orjson` (JSON serialization performance), `pyexiftool` (batch exiftool invocation with safe Unicode argument passing), and `tqdm` (progress reporting), alongside `exiftool` as the sole required external binary.

**G6 — AI-agent implementability.** Every section of this specification provides sufficient detail for an AI implementation agent to produce correct, complete code for the described component within a single context window, without interactive clarification.

**G7 — Structured, v4-native output.** The tool targets the v4 output schema exclusively for new output. The v4 schema adds `relationships[]` on IndexEntry, removes sidecar-origin MetadataEntry fields, and adopts permanent `_idx.json`/`_idxd.json` output suffixes. Legacy v2/v3 output is recognized for backward compatibility in rollback operations.

#### Non-Goals

**NG1 — Full pslib port.** Only the `MakeIndex` function and its dependency tree informed this project.

**NG2 — Backward-compatible v1 JSON output.** The tool does not produce output conforming to the v1 schema.

**NG3 — Drop-in PowerShell replacement.** The tool has its own CLI, API, and configuration system designed for the Python ecosystem.

**NG4 — Real-time or watch-mode indexing.** The tool processes a target path and produces output. It does not monitor the filesystem for changes.

**NG5 — Database or server backend.** Index output is written to JSON files or stdout.
...
**NG7 — Sidecar content ingestion.** The v4 architecture explicitly removes the sidecar ingest/reconstruct/restore pipeline. The tool does not parse sidecar file content, embed it into parent entries, or reconstruct sidecar files during rollback. Sidecar files are first-class index entries with relationship annotations. See the sidecar architecture pivot document for the complete rationale.
...
Output conforms to the v4 schema, which consolidates related fields into typed sub-objects (`NameObject`, `HashSet`, `SizeObject`, `TimestampPair`, `TimestampsObject`, `ParentObject`), adds a `schema_version` discriminator, adds filesystem provenance fields to `MetadataEntry` for MetaMergeDelete reversal, and eliminates the `Encoding` field (see DEV-12). The v4 schema is defined at [schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json](https://schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json). See [§5](#5-output-schema).
...
The `Encoding` field is intentionally absent from the v4 schema. The encoding information it provided (code page identifiers, `IsBrowserDisplay`, `IsMailNewsSave`, etc.) is deeply coupled to the .NET `System.Text.Encoding` type hierarchy and has limited utility outside of .NET consumers. See [§5.11](#511-dropped-and-restructured-fields).
...
#### DEV-17 — Relationship annotation replaces sidecar content ingestion

The v4 architecture removes the entire sidecar content processing pipeline (parse, store, reconstruct) and replaces it with a relationship annotation system. Every file on disk becomes a first-class IndexEntry. The system runs a rule-based relationship classifier over the file inventory and annotates entries with directional relationship data (`relationships[]` array). No file content is parsed and re-serialized. No file is excluded from the main index. No file is embedded into another entry's metadata array. This is a fundamental architectural change driven by the forensic integrity concern documented in the sidecar architecture pivot ([§1.5](#15-reference-documents)).

> **Historical note (DEV-17):** The original MakeIndex and the v2 Python implementation both processed sidecar files through a parse-store-reserialize pipeline. This pipeline was lossy: JSON round-trip through `json.loads()`/`json.dumps()` silently normalizes whitespace, line endings, Unicode escapes, and key ordering. The v4 architecture eliminates this entire class of fidelity concerns by never reading or writing sidecar content.
...
**Rationale for `models/` as a separate subpackage:** In `shruggie-feedtools`, the Pydantic schema models live inside `core/schema.py`. That works for feedtools because its model layer is relatively flat (one `FeedOutput` model with nested item objects). The indexer's model layer is more complex — the v4 schema defines seven distinct sub-object types, the `IndexEntry` itself is recursive (the `items` field contains child `IndexEntry` objects), and the configuration system introduces a parallel set of typed structures (see `config/types.py` below). Separating `models/` from `core/` avoids circular import risks between the configuration types that `core/` modules consume and the schema types that `core/` modules produce. It also provides a clean import path for external consumers who need the types without pulling in the engine: `from shruggie_indexer.models import IndexEntry`.
...
| `conformance/` | Schema conformance tests. Validate that the JSON output produced by the tool conforms to the v2 JSON Schema definition. These tests load the canonical v4 schema from its published URL (or a local copy) and run `jsonschema` validation against actual indexer output. |
...
| `changelog.md` | Auto-copied from `CHANGELOG.md` at the repository root. Contains a header comment (``) identifying it as a generated file. See [§3.7](#37-documentation-site) for the copy mechanism. |
...
| `schema/` | Canonical v2 JSON Schema definition (`shruggie-indexer-v4.schema.json`), a schema overview page (`index.md`), and validation examples (`examples/`). The schema file is the local copy of the canonical schema hosted at [schemas.shruggie.tech](https://schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json). The `examples/` subdirectory contains real-world v2-compliant output files used for manual validation reference and documentation. |
...
**Stage 4 — Entry Construction.** For each path yielded by Stage 3, build a complete `IndexEntry` (the v4 schema object defined in [§5](#5-output-schema)). This is the core of the indexing engine and involves:
...
__init__.py (public API) ─────────┤
              ...
              ┌────────────┘  │  │  │  └────────────┐
              ▼               ▼  │  ▼               ▼
        core/hashing.py  core/paths.py  core/exif.py  core/classifier.py
              │               │  │                    │
              │               │  ▼                    │
              │               │  core/timestamps.py   │
              │               │                       │
...
| `core/entry` → `core/classifier` | File path + parent directory listing + config (patterns) | `Path`, `list[Path]`, `IndexerConfig` → `list[MetadataEntry]` |
...
- `core/classifier.py` reads sidecar file contents from the filesystem. This is a read-only I/O operation.
...
| Logger instances | Created at import time (module-level) | Process lifetime | Mutable (log level can be reconfigured) | Per-module via `logging.getLogger(__name__)` | `core/entry` → `core/classifier` | Directory entry list + config (rules) | `list[IndexEntry]`, `IndexerConfig` → mutates `relationships` on matching entries | Examples | Historical Equivalent |
|------|----------|----------|-----------------------|
...
| Rule set | Stage 1 (configuration resolution) | Entire invocation | Immutable (loaded from config, built-in, and packs) | Passed to classifier via `IndexerConfig` |

---

## 5. Output Schema

This section defines the complete v4 output schema for `shruggie-indexer`. The canonical machine-readable schema is the JSON Schema document at [schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json](https://schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json). This section interprets and extends that schema with implementation guidance and behavioral notes that a JSON Schema cannot express, but does not supersede the canonical schema for structural or type-level definitions.

The v4 schema is the result of the sidecar architecture pivot (see [§1.5](#15-reference-documents)). It introduces `relationships[]` on IndexEntry, removes sidecar-origin content embedding from MetadataEntry, and adopts permanent output suffixes (`_idx.json`, `_idxd.json`) decoupled from the schema version number.

### 5.1. Schema Overview

#### Design principles

The v4 schema inherits the five v2 design principles (P1 through P5, see the v2 specification archive for the full descriptions) and adds a sixth:

**P6 — Relationship annotation without content ingestion.** The indexer annotates believed associations between files (e.g., "this `.info.json` is metadata for that `.mp4`") via a `relationships[]` array on the sidecar-like file's IndexEntry. The annotation is purely informational: it does not change how either file is processed, stored, renamed, or rolled back. No file content is parsed, stored, or re-serialized by the indexer. Sidecar files are first-class IndexEntry objects with their own identity, timestamps, and metadata.

#### Canonical schema location

```
https://schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json
```

This document uses JSON Schema Draft-07. The schema `$id` is set to the canonical URL. The schema `title` is `IndexEntry`.

#### Schema version

Every v4 output document includes a `schema_version` field with the integer value `4`. Consumers SHOULD check this field before attempting to parse the document and SHOULD reject documents with an unrecognized schema version.

### 5.2. Reusable Type Definitions

> **Updated 2026-03-20:** Added `EncodingObject` (§5.2.7) for v3 encoding metadata.

The v4 schema defines seven reusable type definitions in the `definitions` block of the JSON Schema. These are the building blocks from which the top-level `IndexEntry` properties and the nested `MetadataEntry` objects are composed. Each definition is referenced via `$ref` wherever it appears.
...
An `IndexEntry` is a JSON object conforming to the root schema. It describes a single indexed file or directory. The following table lists all top-level properties.

| Property | Type | Required | Section |
|----------|------|----------|---------|
| `schema_version` | `integer` (const `4`) | Yes | [§5.4](#54-identity-fields) |
...
| `relationships` | `array` of `RelationshipAnnotation` | No | [§5.11](#511-relationship-annotation-fields) |

The `required` array in the canonical schema is:
...
The `items`, `metadata`, `session_id`, `indexed_at`, `mime_type`, and `relationships` fields are not in the `required` array. The `relationships` field is **absent** (not empty, not null) when no relationships are detected for a file. When present, it is a non-empty array of `RelationshipAnnotation` objects.
...
> **Updated 2026-03-20:** Bumped from `const: 4` to `const: 3` for v0.2.0.
...
| Required | No |
| v2 equivalent | `metadata` (structurally simplified) |

An array of metadata records associated with the indexed item. In the v4 architecture, the `metadata` array contains **only** ExifTool-generated entries (when the `--meta` flag is active and the file type is not in the exclusion list). Sidecar metadata content is no longer ingested into this array. For directories, `metadata` is `null`.

The distinction between `null` (metadata processing not requested) and `[]` (metadata processing was performed but produced no results) is semantically meaningful and SHOULD be preserved.

#### MetadataEntry (v4)

The v4 `MetadataEntry` is structurally simplified compared to v2. All sidecar-origin fields are removed because the indexer no longer ingests sidecar content.

**Top-level properties of `MetadataEntry`:**

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `id` | `string` | Yes | Unique identifier. Prefix `z` for generated. Pattern: `^z[0-9A-F]+$`. |
| `origin` | `string` (enum) | Yes | `"exiftool"`. The `"sidecar"` origin value is retired in v4. |
| `name` | `NameObject` | Yes | For generated: `text` and `hashes` are both `null`. |
| `hashes` | `HashSet` | Yes | Content hashes of the serialized output. |
| `attributes` | `object` | Yes | Classification and format info. |
| `data` | `object` or `null` | Yes | The metadata content. For ExifTool: a JSON object. |

The `required` array for `MetadataEntry` is: `["id", "origin", "name", "hashes", "attributes", "data"]`.

**Removed v2 MetadataEntry fields (no longer applicable in v4):**

| Removed field | v2 purpose | v4 disposition |
|---------------|------------|----------------|
| `file_system` | Sidecar file relative path for reversal | Removed. No sidecar content to reverse. |
| `size` | Sidecar file size for reversal | Removed. |
| `timestamps` | Sidecar file timestamps for reversal | Removed. |
| `attributes.format` | `"json"`, `"text"`, `"base64"`, `"lines"` | Removed. ExifTool output is always JSON. |
| `attributes.transforms` | `["base64_encode"]`, `["json_compact"]`, etc. | Removed. No content transformation pipeline. |
| `attributes.source_media_type` | MIME type of binary sidecar data | Removed. |

The `attributes` sub-object in v4 is simplified:

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `type` | `string` | Yes | `"exiftool.json_metadata"` for ExifTool output. |

The `"sidecar"` origin value is retired because sidecar files are no longer absorbed into parent entries. They are indexed as independent entries with their own identity and (when detected) a `relationships[]` annotation pointing at their parent. The sidecar type vocabulary (`description`, `json_metadata`, `thumbnail`, etc.) moves to the `type` field on `RelationshipAnnotation` (see [§5.11](#511-relationship-annotation-fields)).

### 5.11. Relationship Annotation Fields

#### `relationships`

| Attribute | Value |
|-----------|-------|
| Type | `array` of `RelationshipAnnotation` |
| Required | No |
| v2 equivalent | None (new in v4) |

An array of directional relationship annotations. Each element describes a believed association between this IndexEntry and another indexed file. The `relationships` array is **absent** (not empty, not null) when no relationships are detected for a file. Its presence indicates that at least one sidecar rule matched this file's name.

Relationships are directional: the sidecar-like file's entry points at the parent-like file. The parent receives no back-reference. Bidirectional discovery, graph inversion, and consolidated asset views are catalog responsibilities (see the metadexer specification).

#### RelationshipAnnotation

| Property | Type | Required | Description |
|----------|------|----------|-------------|
| `target_id` | `string` | Yes | The `id` of the related IndexEntry (the "parent" file). |
| `type` | `string` | Yes | The semantic relationship type. |
| `rule` | `string` | Yes | The name of the sidecar rule that produced this relationship. Enables auditability and debugging. |
| `rule_source` | `string` | Yes | The origin of the rule: `"builtin"`, `"user"`, or `"pack:<pack-name>"`. Provides provenance without encoding it into the rule name. |
| `confidence` | `integer` | Yes | How confident the classifier is. See confidence levels below. |
| `predicates` | `array` | Yes | Detailed predicate evaluation results. See predicate detail below. |

#### Relationship types

The initial set of relationship types matches the sidecar type vocabulary from the v2 configuration:
...
No relationship entry is written when no rule matches a file. Absence of a `relationships` array indicates no rules applied.

#### Predicate detail

Each relationship entry includes a `predicates` array documenting the evaluation outcome of each predicate defined on the matching rule.
...
### 5.12. Dropped and Restructured Fields
...
**Removed from MetadataEntry:**

| v2 Field | v4 Disposition |
|----------|----------------|
| `MetadataEntry.origin: "sidecar"` | Retired. Sidecar files are indexed as independent entries, not absorbed. |
| `MetadataEntry.file_system` (sidecar provenance) | Removed. No sidecar content ingestion. |
| `MetadataEntry.size` (sidecar provenance) | Removed. |
| `MetadataEntry.timestamps` (sidecar provenance) | Removed. |
| `MetadataEntry.attributes.format` | Removed. Only ExifTool JSON remains. |
| `MetadataEntry.attributes.transforms` | Removed. No content transformation pipeline. |
| `MetadataEntry.attributes.source_media_type` | Removed. |
| `MetadataEntry.data` (sidecar content) | Removed for sidecar entries. ExifTool `data` field remains. |

**Added to IndexEntry:**

| v4 Field | Description |
|----------|-------------|
| `relationships` | Array of directional relationship annotations. See [§5.11](#511-relationship-annotation-fields). |

**Suffix convention change:**

| Scope | v2 suffixes | v4 suffixes (permanent) |
|-------|-------------|------------------------|
| File-level | `_meta2.json` | `_idx.json` |
| Directory-level | `_directorymeta2.json` | `_idxd.json` |
...
### 5.13. Schema Validation and Enforcement

#### Build-time validation

> **Updated 2026-03-20:** The validation target is now the v4 schema. The encoding conditional-inclusion invariant has been added to the serialization invariants.

The canonical v3 JSON Schema (`shruggie-indexer-v4.schema.json`) MUST be used as the validation target for output conformance testing (see [§14.4](#144-output-schema-conformance-tests)). The test suite SHOULD include a schema conformance test that:
...
The canonical v4 JSON Schema (`shruggie-indexer-v4.schema.json`) MUST be used as the validation target for output conformance testing.

**Additional serialization invariant for v4:**

6. **Relationship field presence.** The `relationships` field MUST be absent (not empty, not `null`) when no relationships are detected. When present, it MUST be a non-empty array of valid `RelationshipAnnotation` objects. This is a structural invariant; the classifier never produces an empty array.

### 5.14. Backward Compatibility Considerations

The v4 schema is a breaking change from v2. Consumers of existing v2 index assets cannot parse v4 output without modification, and vice versa.

#### In-place output suffix convention

Starting with v4, the in-place output files use fixed suffixes decoupled from the schema version number:
...
A `cleanup_legacy_sidecars` config key (default: `false`) and corresponding CLI flag (`--cleanup-legacy-sidecars`) control automatic removal of obsolete tool output files. When enabled:

1. After a successful index run that writes `_idx.json` and/or `_idxd.json` files, the system scans the same directories for old-format siblings.
2. An old-format file is deleted only if a new-format file was successfully written in the same run for the same scope.
3. Orphaned legacy files in directories the current run did not touch are never deleted.
4. Each deletion is logged at `INFO` level. No user confirmation is required.
...
The rollback engine MUST continue to support v2 and v3 sidecar files for rollback of existing indexed collections. When loading v2/v3 output from `_meta2.json` or `_meta3.json` files, the legacy `_decode_sidecar_data()` path is retained as a fallback. New v4 output written to `_idx.json` never triggers this path.

#### Consumer guidance

Consumers adapting from v2 to v4 parsing should:

1. Check `schema_version`. If `4`, parse as v4.
2. Handle the new `relationships` field on IndexEntry (may be absent).
3. Expect `metadata[]` to contain only ExifTool entries (no sidecar-origin entries).
4. Update MetadataEntry parsing to remove sidecar-specific field handling.
5. Note the new output file suffixes (`_idx.json`, `_idxd.json`).
...
|-----------|---------------------|---------|
| File | `{parent_dir}/{filename}_idx.json` | `photos/sunset.jpg` → `photos/sunset.jpg_idx.json` |
| Directory | `{directory}/{dirname}_idxd.json` | `photos/vacation/` → `photos/vacation/vacation_idxd.json` |
...
### 6.7. Relationship Classification

**Module:** `core/classifier.py`
**Operations Catalog:** Category 7
**Replaces:** `core/sidecar.py` (v2 sidecar discovery, parsing, and MetadataEntry construction)

#### Purpose

Examines the file inventory after the traversal and entry construction passes and annotates IndexEntry objects with directional relationship data. The classifier determines which files are likely sidecars of other files based on configurable naming rules, then populates the `relationships[]` array on matching entries. It does NOT read, parse, or store any file content.

This module replaces the entire v2 sidecar processing pipeline: `GetFileMetaSiblings` discovery, `ReadMetaFile`/`MetaFileRead` parsing, MetadataEntry construction, and the MetaMergeDelete queue. See the sidecar architecture pivot ([§1.5](#15-reference-documents)) for the full rationale.
...
def classify_relationships(
    ...
    """Annotate entries with relationship data.

    For each directory in the entry tree, examines all file entries
    and matches their names against the configured sidecar rules.
    When a rule matches, populates the `relationships` field on the
    matching entry with a RelationshipAnnotation pointing at the
    inferred parent entry.

    Mutates entries in place (sets the `relationships` attribute).
    Does not return a value.

    Args:
        entries: The list of IndexEntry objects for a single directory's
                 immediate children. Called once per directory during
                 the traversal.
        config: Resolved configuration containing the loaded rule set
                (built-in + user + pack rules).
    """
```

#### Classification algorithm

For each directory being indexed, the classifier operates on the list of file entries in that directory:

1. **Build a stem-to-entry map.** For each file entry, extract the stem (filename without final extension) and map it to the entry's `id`. This enables O(1) lookup when a rule's `{stem}` token resolves to a candidate parent.

2. **Iterate file entries.** For each file entry in the directory:

   a. **Match rules.** Test the file's name against each rule in the rule set (user rules first, then pack rules, then built-in rules; see [§7.5](#75-sidecar-rule-engine) for resolution order). Use the rule's `match` pattern with `{stem}` and `*` token expansion. First matching rule wins.

   b. **Resolve target.** If a rule matches, identify the target entry (the "parent" file). For file-scoped rules (`scope: "file"`), the target is the entry whose stem matches the `{stem}` token binding. For directory-scoped rules (`scope: "directory"`), the target is the directory's own IndexEntry.

   c. **Evaluate predicates.** If the matching rule defines predicates (`requires_sibling`, `requires_sibling_any`, `excludes_sibling`), evaluate each predicate against the directory's file list. Record each predicate's outcome (name, pattern/patterns, satisfied: true/false).

   d. **Compute confidence.** If all predicates are satisfied (or no predicates are defined): `confidence = 3`. If some predicates are satisfied: `confidence = 2`. If no predicates are satisfied: `confidence = 1`.

   e. **Build annotation.** Construct a `RelationshipAnnotation` with `target_id`, `type` (from the rule), `rule` (rule name), `rule_source` (`"builtin"`, `"user"`, or `"pack:<name>"`), `confidence`, and `predicates` (the evaluation detail array).

   f. **Attach to entry.** Set the `relationships` field on the current file's IndexEntry. An entry MAY have multiple relationships if multiple rules match (e.g., a file might be both a thumbnail and a screenshot depending on the rules). In the standard resolution model (first match wins), each file gets at most one relationship.

3. **Skip tool artifacts.** Files matching `metadata_exclude_patterns` are not present in the entry list (they were excluded during traversal). The classifier never sees them.

#### What the classifier does NOT do

- Does NOT read file content. No `open()`, no `json.loads()`, no Base64 encoding.
- Does NOT create MetadataEntry objects. The `metadata[]` array is populated only by ExifTool.
- Does NOT maintain a delete queue. No files are deleted.
- Does NOT modify any field on IndexEntry other than `relationships`.

#### Sidecar-blind mode

When `--no-sidecar-detection` is active (or `config.no_sidecar_detection = True`), the classifier is not invoked. Every file is indexed as a standalone content file with no `relationships` field. The rule engine is not loaded.

#### Error handling

The classifier operates on in-memory data with no I/O. The only anticipated error is a misconfigured rule that produces an invalid `{stem}` binding (no matching parent found). This is not an error condition; the rule simply does not match, and no relationship is produced. Diagnostic-level logging notes the non-match for debugging purposes.
...
def build_file_entry(
    path: Path,
    config: IndexerConfig,
    siblings: list[Path] | None = None | None = None,
) -> IndexEntry:
    ...
    Args:
        ...
        siblings: Pre-enumerated sibling files in the same directory
                  ...
                  enumerate the parent directory. accumulator (see §6.7).

    Returns a fully populated IndexEntry conforming to the v4 schema.
    """

def build_directory_entry(
    path: Path,
    config: IndexerConfig,
    recursive: bool = False | None = None,
    ...
    Args:
        ...
        config: Resolved configuration.
        recursive: Whether to descend into subdirectories. accumulator.
...
**Step 14 — Relationship classification.** After all file and directory entries for a directory are constructed, invoke `classifier.classify_relationships(child_entries, config)`. This runs once per directory. The classifier examines the complete sibling list and populates `relationships[]` on entries where rules match. This call is a no-op if `config.no_sidecar_detection` is `True`.
...
> **Updated 2026-02-25:** Documents the `{dirname}_idxd.json` naming convention for in-place directory sidecars.
...
After the Stage 6 consumed-sidecar deletion, the MetaMergeDelete pipeline performs a final cleanup pass to remove stale metadata artifacts from prior indexer runs. These are files matching the `metadata_exclude_patterns` regex (`_meta.json`, `_idx.json`, `_meta3.json`, `_directorymeta.json`, `_idxd.json`, `_directorymeta3.json`) that were excluded from traversal at Layer 1 ([§7.5](#75-sidecar-suffix-patterns-and-type-identification)) and therefore never entered 
...
**Module:** `core/rollback.py`
**New in v4.**

#### Purpose

Restores renamed files to their original names and locations. In the v4 architecture, rollback is uniform for all files: every entry in the rollback plan is a file copy (or re-copy for duplicates). The source file exists on disk in its storage-named form. The rollback engine copies it to the original relative path and restores timestamps.

#### Simplified behavior

Every file has an IndexEntry with `name.text` (original filename) and `file_system.relative` (original relative path). Rollback reads the `_idx.json` sidecar file for each renamed file, extracts the original path information, and performs the reverse rename. No sidecar reconstruction. No format-aware re-serialization. No encoding-aware text restoration. Byte-perfect by construction.

#### Public interface

```python
def rollback_from_index(
    index_path: Path,
    config: IndexerConfig,
    *,
    dry_run: bool = False,
    ...
    Reads the _idx.json sidecar file (or _idxd.json for directories)
    and reverses the storage-name rename for each entry.

    Args:
        index_path: Path to the _idx.json or _idxd.json file, or to
            the directory containing in-place sidecars.
        config: Resolved configuration.
        dry_run: When True, compute and report the rollback plan
            without executing any renames.

    Returns a RollbackResult containing the list of files
    processed, files skipped, and any errors encountered.
    """
```

#### Backward compatibility

The rollback engine MUST continue to support v2 and v3 sidecar files. When loading from `_meta2.json` or `_meta3.json` files:

1. Detect the schema version from the `schema_version` field.
2. If v2 or v3: invoke the legacy `_decode_sidecar_data()` path for sidecar-origin MetadataEntry objects that need content reconstruction.
3. If v4: all entries roll back as simple file copies.

The legacy codepaths (`_decode_sidecar_data()`, `_restore_json()`, `_apply_text_encoding()`) are retained exclusively for v2/v3 backward compatibility. They are never invoked for v4 output.
...
> **v4 update:** `MetadataTypeAttributes` and `ExiftoolConfig` nested dataclasses removed. `SidecarRule` dataclass added — see §5.11 and §7.5.
...
For example, `meta_merge_delete=True` with all other flags at their defaults produces `meta_merge=True, extract_exif=True, meta_merge_delete=True`.
...
1. **IdType validity.** `id_algorithm` MUST be either `"md5"` or `"sha256"`. Validation occurs during configuration construction.

   > **Historical note:** The original validates this with a `switch` statement that falls through to an error (line ~8776).

2. **Path conflicts.** If `output_file` is specified, it MUST NOT point to a path inside the target directory when `output_inplace` is also active. (Writing the aggregate output file into the same directory tree being indexed with in-place writes active would cause the aggregate file to be indexed on subsequent runs.)

3. **Regex compilation.** All regex strings in the metadata identification and exclusion pattern lists MUST compile without error. If a user-provided pattern is invalid, the loader raises a `ConfigurationError` with the offending pattern and the `re.error` message.
...
> **Note (v4):** The `Identify` → `metadata_identify` mapping documented here is retained as reference material for the built-in sidecar rule library, but the runtime system uses the TOML-based rule engine (§7.5) rather than compiled regex patterns. The `Attributes` → `metadata_attributes` mapping and `MetadataTypeAttributes` dataclass are retained for documentation purposes only; they are not used at runtime in v4.
...
### 7.5. Sidecar Rule Engine

**Replaces:** §7.5 (Sidecar Suffix Patterns and Type Identification) from the v2 specification.

This subsection specifies the TOML-based rule engine that drives the relationship classifier ([§6.7](#67-relationship-classification)). The rule engine replaces the v2 `metadata_identify` regex system and the `MetadataFileParser.Identify` configuration.
...
The `{stem}` token is the key innovation: it binds the sidecar's identity to a specific content file's name, which is the relationship the rule is asserting. If a user needs full regex (advanced use case), a future `match_regex` field can coexist alongside `match` without changing the default experience.
...
|-----------|------|---------|
| `requires_sibling` | string (pattern) | Full confidence only if a file matching this pattern exists in the same directory. |
...
| `excludes_sibling` | string (pattern) | NOT at full confidence if a file matching this pattern exists. |

Predicate patterns use the same token syntax as `match` (`{stem}`, `*`, literals).

**Predicate failure behavior:** When a rule's pattern matches but one or more required predicates fail, the relationship is emitted with a reduced `confidence` value (2 for partial satisfaction, 1 for no predicates satisfied). The relationship is never silently suppressed. The `predicates` array on the relationship entry documents which predicates passed and which failed.

#### Scope

The `scope` field (default: `"file"`) controls what the rule's `{stem}` token binds to:

- `"file"`: The `{stem}` token matches against content file stems in the same directory. Standard case.
- `"directory"`: The rule matches files that are sidecars of the directory context itself (e.g., `desktop.ini`, `folder.jpg`). The `{stem}` token is not available. The `target_id` in the resulting relationship points to the directory's IndexEntry.
...
Installed packs are loaded after user rules and before built-in rules. If multiple packs define rules with the same name, load order is alphabetical by pack filename, and the first definition wins. User rules always override pack rules.

The pack management CLI is a future enhancement (see [§18](#18-future-considerations)). For the initial implementation, packs are manually placed in the directory.

#### metadata_exclude_patterns update

The `metadata_exclude_patterns` list is updated to cover all legacy and new suffixes:

```python
METADATA_EXCLUDE_PATTERNS = (
    re.compile(r'_(meta[23]?|directorymeta[23]?)\.json$', re.IGNORECASE),
    re.compile(r'_(idx|idxd)\.json$', re.IGNORECASE),
    re.compile(r'\.(cover|thumb|thumb(s|db|index|nail))$', re.IGNORECASE),
    re.compile(r'^(thumb|thumb(s|db|index|nail))\.db$', re.IGNORECASE),
)
```

Files matching these patterns are excluded from both the entry list and the relationship classifier. They are invisible to the entire pipeline.
...
| Base field | Append variant |
|-----------|----------------|
| `sidecar_rules.<name>` | (individual rules are named; adding a new rule is defining a new `[sidecar_rules.<name>]` table) |

Rules are merged by name. A user rule with the same name as a built-in rule replaces it entirely. A user rule with `enabled = false` and `extends = "<builtin-name>"` disables the named built-in.
...
Controls whether `_idxd.json` directory sidecar files are written during in-place output. The flag is a boolean pair: `--dir-meta` enables directory metadata (the default behavior), `--no-dir-meta` suppresses it. When `None` (not specified on the command line), the value is resolved from the configuration file or compiled default (`True`).

Suppression affects only directory-level sidecars — per-file `_idx.json` sidecars are always written when `--inplace` is active. Auto-generated aggregate output files ending in `_idxd.json` are also suppressed. Explicitly specified `--outfile` paths are never suppressed. Stdout output is never affected. See [§6.9](#69-json-serialization-and-output-routing), "Directory metadata suppression" for the full routing table.

Maps to `config.write_directory_meta` ([§7.1](#71-configuration-architecture)).

### 8.4. Metadata and Relationship Processing Options

> **Updated 2026-04-01:** Section retitled from "Metadata Processing Options." The `--meta-merge` and `--meta-merge-delete` flags are removed. New flags `--no-sidecar-detection` and `--cleanup-legacy-sidecars` are added.

| Flag | Type | Default | Description |
|------|------|---------|-------------|
| `--meta` / `-m` | Boolean | `False` | Enable ExifTool metadata extraction. Invokes `exiftool` for each eligible file and stores results in the `metadata[]` array. |
| `--no-sidecar-detection` | Boolean | `False` | Disable the relationship classifier entirely. Every file is indexed as a standalone content file with no `relationships` array. The rule engine is not invoked. |
| `--cleanup-legacy-sidecars` | Boolean | `False` | After a successful index run, remove old-format tool output files (`_meta.json`, `_meta2.json`, `_meta3.json`, `_directorymeta.json`, `_directorymeta2.json`, `_directorymeta3.json`) in directories where new-format files (`_idx.json`, `_idxd.json`) were successfully written. |

**Removed flags:**

| Removed flag | v2 purpose | v4 disposition |
|--------------|------------|----------------|
| `--meta-merge` / `-mm` | Fold sidecar metadata content into parent entries | Removed. No sidecar content ingestion in v4. |
| `--meta-merge-delete` / `-mmd` | Merge sidecar content then delete sidecar files | Removed. No sidecar content ingestion in v4. |
...
def serialize_entry(
    ...
    *,
    ...
    Returns:
        A UTF-8 JSON string conforming to the v4 schema.
    """
```
...
> **v4 update:** `RelationshipAnnotation` dataclass added (see §5.11). `SidecarRule` dataclass added for the rule engine. `MetadataEntry` simplified (sidecar-origin fields removed). `MetadataTypeAttributes` removed.
...
@dataclass
class IndexEntry:
    """A single indexed file or directory (v4 schema)."""
...
| Multi-file | `multi` | Writes per-item in-place sidecar JSON files alongside each processed item (`_idx.json` / `_idxd.json`). |
...
|-------------|-------------------|---------|
| File | `{parent_dir}/{filename}_idx.json` | Target: `C:/my/cool/file.txt` → Output: `C:/my/cool/file.txt_idx.json` |
| Directory | `{directory_path}_idxd.json` | Target: `C:/my/cool/dir` → Output: `C:/my/cool/dir_idxd.json` |

**Edge case handling:**

| Target path | Behavior |
|-------------|----------|
| `C:/` | Strip trailing separator, produce `C:/_idxd.json`. |
| `/` (Unix root) | Fall back to user's home directory: `~/root_idxd.json`. |
...
> **Updated 2026-04-01:** Meta-merge controls removed. No new GUI controls for the relationship classifier (built-in rules apply automatically; custom rules are configured via the TOML configuration file).
...
**Attribution line:** "Built by Shruggie LLC (DBA ShruggieTech)" is displayed at the bottom of the tab in a muted font.
...
1. **Default output behavior** MUST be displayed in the UI as descriptive text in the read-only output path display (e.g., the computed path `target_idxd.json` or `"(displayed in viewer)"` for View only mode).
...
| `dataclasses` | `config/types.py`, `models/schema.py` | Frozen dataclass definitions for `IndexerConfig`, `IndexEntry`, and all v4 schema sub-objects. `dataclasses.asdict()` used by the serializer ([§6.9](#69-json-serialization-and-output-routing)). |
...
- All sidecar round-trip fidelity tests (the 12 tests covering `.url` text cascade, `.lnk` dual-storage, JSON style detection, and restoration fidelity). The `.lnk` and `.url` specific handling is no longer needed; they are ordinary files.
- Meta-merge and meta-merge-delete integration tests.
- Sidecar content reconstruction tests.
- `test_sidecar.py` is replaced by `test_classifier.py`.

**Tests added:**

- **Relationship classifier unit tests** (`test_classifier.py`): rule matching with `{stem}` and `*` tokens, predicate evaluation (requires_sibling, requires_sibling_any, excludes_sibling), confidence assignment (3/2/1), predicate detail recording, rule resolution order (user > pack > builtin), rule disabling via `enabled = false`.
- **Rollback unit tests** (`test_rollback.py`): uniform file copy rollback, timestamp restoration, legacy v2/v3 sidecar file rollback via the fallback path.

### 14.3. Integration Tests

**Tests added:**

- **Relationship integration tests** (`test_relationship_integration.py`): full index of a directory containing known sidecar conventions (yt-dlp output, `.lnk` shortcuts, `.url` files, thumbnail images), verifying correct relationship annotations with expected confidence values and predicate outcomes.
- **Rollback integration tests**: verify that files previously treated as sidecars (in v2) are now rolled back as ordinary files (byte-perfect round-trip).
...
> **Updated 2026-03-20:** Conformance tests now target the v3 schema (`shruggie-indexer-v4.schema.json`). The v2 schema fixture is retained for backward-compatibility validation.

Conformance tests validate that the JSON output produced by the indexer structurally matches the canonical v3 JSON Schema definition at `schemas.shruggie.tech/data/shruggie-indexer-v4.schema.json`. These tests use the `jsonschema` package ([§12.3](#123-third-party-python-packages)) to perform Draft-07 validation against actual serializer output.
...
SCHEMA_PATH = Path(__file__).parent.parent / "fixtures" / "shruggie-indexer-v4.schema.json"
...
def validate_entry(entry_json: str, schema: dict) -> None:
    """Validate a JSON string against the v4 schema. Raises on failure."""
...
|-----------|-------|------------|
| Single file entry | Index a text file, serialize to JSON | `jsonschema.validate()` passes against the v4 schema. |
...
sidecar_path = item_path.parent / f"{item_path.name}_idx.json"
```
...
The indexer does not validate filename lengths. If the filesystem accepts a filename, the indexer can process it. Filenames that exceed the target filesystem's limit when writing sidecar files (the original name plus `_idx.json` suffix, or `_idxd.json`) will produce an `OSError` that is handled as a file-level error ([§4.5](#45-error-handling-strategy)).
...
**In-place sidecar files (`--inplace`).** Sidecar paths are constructed by `paths.build_sidecar_path()` ([§6.2](#62-path-resolution-and-manipulation)), which appends a fixed suffix (`_idx.json` or `_idxd.json`) to the item's existing path. The construction uses `pathlib` path arithmetic (`item_path.parent / sidecar_name`), not string concatenation. The sidecar filename is derived deterministically from the item filename, so there is no opportunity for path injection — a filename like `../../etc/passwd` on the filesystem would produce a sidecar named `../../etc/passwd_idx.json` in the same directory (not a traversal to `/etc/`), because `pathlib`'s `/` operator joins relative to the parent, not relative to the working directory.
...
> **Updated 2026-04-01:** Former §16.4 (Metadata Merge-Delete Safeguards) is removed entirely. The MetaMergeDelete feature is eliminated in the v4 architecture. No sidecar files are deleted during indexing. The safety concern it addressed (preventing data loss from deleting sidecar files without capturing their content) is moot because sidecar files are never deleted.

### 16.4. Large File and Deep Recursion Handling
...
The following items are enabled by the v4 architecture but are not part of the initial implementation:

- **Community pack CLI** (`packs list-remote`, `packs install`, `packs list`, `packs remove`). A CLI subcommand for managing sidecar rule packs. See [§7.5](#75-sidecar-rule-engine).
- **Sidecar audit mode** (`--sidecar-audit` flag). Runs the full index with relationship classification enabled but produces an additional audit report listing every relationship detected, every unmatched file, and every near-miss (pattern matched but predicate failed).
- **Interactive training mode**. Guided rule authoring from audit output. A user-facing workflow for iteratively refining sidecar detection rules.
- **GUI rule editor** and relationship visualization panel.
- **`match_regex` field** on rules for advanced users who need full regex power.
- **`min_siblings` predicate** for cluster detection (the relationship is produced at full confidence only if at least N other files in the directory also match rules pointing at the same target).

### 18.2. Schema Evolution

The v4 schema uses permanent output suffixes (`_idx.json`, `_idxd.json`). Future schema version bumps change the `schema_version` field inside the file, not the filename. This eliminates the suffix-churn pattern from prior versions and simplifies the `metadata_exclude_patterns` configuration.

The `relationships[]` array is designed as an open-ended extension point. New relationship types, new predicate types, and new annotation fields can be added in future schema versions without structural changes to the array shape.
