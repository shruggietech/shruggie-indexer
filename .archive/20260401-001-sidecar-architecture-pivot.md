...
**Document ID:** `20260401-001-sidecar-architecture-pivot.md`
**Status:** Active
**Supersedes:** [`20260321-005-sidecar-architecture-pivot.md`](https://github.com/shruggietech/shruggie-indexer/blob/main/.archive/20260321-005-sidecar-architecture-pivot.md) (Ratified 2026-03-24)
**Scope:** Defines a fundamental architectural change to how `shruggie-indexer` handles sidecar files. Implications cascade into the `metadexer` technical specification, the `shruggie-indexer` technical specification, the v4 output schema, and the metadexer catalog module.
...
- [`20260321-005-sidecar-architecture-pivot.md`](https://github.com/shruggietech/shruggie-indexer/blob/main/.archive/20260321-005-sidecar-architecture-pivot.md) — The original ratified pivot document. All architectural decisions, schema design, and implementation sequencing from that document are carried forward here unchanged.
- **ShruggieTech Unified Product Vision** (v1.0.0, 2026-03-30) — Establishes a three-layer architecture (open-source foundation, internal infrastructure, commercial products) with metadexer as the keystone project. Explicitly names the sidecar architecture pivot and v4 schema as Phase 1 deliverables on the critical path.
- **Metadexer Technical Specification** (DRAFT, 2026-03-19) — Defines the vault, catalog, and sync modules that consume IndexEntry output. Currently references IndexEntry v2 throughout and requires updating to v4.

This document is a new edition that incorporates strategic context from the two documents above into the ratified pivot. It adds Section 9 (Strategic Context), expands the ecosystem impact analysis in Section 10, and corrects all references from "shruggie-catalog" to "metadexer catalog module" for terminological consistency with the consolidated architecture.
...
Every new edge case requires a new compensating field on the schema and new restoration logic. This is a game of whack-a-mole.
...
Add trailing-newline detection and fix the CRLF application order in `_apply_text_encoding()`. Rejected as treating a symptom. The next edge case would require another patch.
...
### 3.4. Remove the sidecar processing system entirely (SELECTED)
...
## 8. Impact on Existing shruggie-indexer Systems
...
- All sidecar round-trip fidelity tests (the 12 tests covering `.url` text cascade, `.lnk` dual-storage, JSON style detection, and restoration fidelity). The `.lnk` and `.url` specific handling is no longer needed; they are just files.
...
This section places the sidecar architecture pivot within the broader ShruggieTech product strategy as defined by the Unified Product Vision (v1.0.0, 2026-03-30).

### 9.1. Position in the three-layer architecture

The product vision organizes the ShruggieTech ecosystem into three layers: an open-source foundation, internal infrastructure, and commercial products. shruggie-indexer and metadexer are both Layer 1 (open-source foundation) projects. The indexer defines content identity. metadexer manages everything downstream of identification: storage, cataloging, search, and (in the planned expansion) semantic retrieval and extraction primitives.

The sidecar architecture pivot is listed as a Phase 1 (Current Quarter) deliverable in the product vision's dependency chain, alongside metadexer Phase 2 (Storage and Catalog). These two workstreams run concurrently because the indexer's v4 schema and the catalog's ingestion pipeline are co-dependent: the catalog cannot finalize its relationship graph implementation until the v4 schema is stable, and the indexer's v4 output is only fully exercised once the catalog consumes it.

### 9.2. Metadexer as keystone

The product vision identifies metadexer as the keystone project in the portfolio. The majority of the commercial roadmap (Covarity, the personal knowledge product) and internal infrastructure (MCP Knowledge Server Tier 2, AI Gateway retrieval) depend on metadexer's catalog, search, and extraction capabilities. Development decisions that accelerate metadexer's critical path have outsized impact on the entire roadmap.

This has a concrete implication for the sidecar pivot: the v4 schema is not just an indexer-internal refactor. It reshapes the IndexEntry contract that metadexer consumes. The metadexer technical specification (DRAFT, 2026-03-19) currently references IndexEntry v2 throughout. The following sections of the metadexer spec require updates to reflect v4:

- §1.5 (Terminology): "IndexEntry" definition references v2.
- §2.2 (Relationship to shruggie-indexer): References v2 schema and v0.1.2 as current stable.
- §3.1 (Component Map): Diagram reads "produces IndexEntry v2 JSON."
- §3.3 (Data Flow): Step 2 references IndexEntry records without version context. The new step will produce records that include `relationships[]` data; the sync module must decide how to handle it.
- §4 (The IndexEntry Contract): All subsections reference v2.
- §6.4.2 (CatalogIngestor): `schema_version == 2` validation hardcoded in the ingest procedure.
- §6.8 (Catalog Database Schema): The `schema_version` column stores `2`; the `raw_entry` column stores the complete IndexEntry dict. The relationship data arrives inside that dict and is available for downstream indexing without schema changes, but the catalog should project relationship data into queryable structures (see Section 10.1).

### 9.3. Enabling the extraction pipeline

The product vision describes a general-purpose extraction pipeline planned for metadexer's ingestion path: chunking, embedding computation, entity extraction, relationship extraction, temporal anchoring, and classification/tagging. These are domain-agnostic capabilities exposed through a plugin surface. Domain-specific products (Covarity, the personal knowledge product) supply their own extraction configurations while metadexer provides orchestration, provenance tracking, and output parsing.

The v4 relationship annotations directly feed this pipeline. When the catalog builds and persists the relationship graph from the indexer's directional annotations (see Section 10.1), it produces the structured context that extraction agents need. A Covarity ingestion agent processing a video file can query the catalog for all related files (subtitles, descriptions, thumbnails, metadata JSON) and use that cluster as input for cross-document entity extraction and correlation. Without the v4 relationship graph, the extraction pipeline would have to re-derive file associations from filename heuristics at ingestion time, duplicating work the indexer already performed.

The confidence levels and predicate detail on each relationship annotation (Sections 5.3 and 5.4) give the extraction pipeline a quality signal. A `confidence: 3` relationship can be trusted without further validation. A `confidence: 1` relationship might warrant a secondary check (e.g., content-based similarity) before the extraction pipeline relies on it. This graduated confidence model aligns with the product vision's principle of building general-purpose capabilities with domain-specific configuration surfaces.

### 9.4. Consolidation implications

The product vision documents several consolidation decisions, most notably the absorption of shruggie-memex into metadexer as a semantic retrieval module. shruggie-memex was conceived as a standalone MCP+RAG service with its own chunking, embedding, and retrieval stack. That capability now belongs inside metadexer's catalog module.

This consolidation reinforces the catalog's expanded scope described in Section 10.1. The catalog becomes the system that knows about file relationships (from v4 annotations), file content (from vault-stored inline text), and semantic similarity (from planned vector embeddings). These three capabilities compose: a search query can find files by metadata, then expand results to include related files via the relationship graph, then rank by semantic similarity to a natural-language query. Each capability is built once and serves every downstream consumer.

---

## 10. Impact on Ecosystem

### 10.1. Indexer-catalog contract

The contract between the indexer and the metadexer catalog module changes fundamentally.
...
The `raw_entry` column on the catalog's `assets` table already stores the complete IndexEntry dict, so relationship data arrives in the catalog without schema changes. The catalog SHOULD project relationship data into dedicated queryable structures (a `relationships` table or equivalent) to support efficient graph traversal and bidirectional lookups. This projection is a catalog implementation detail, not an IndexEntry contract concern.

### 10.2. Metadexer specification update scope

The metadexer spec requires a focused update pass to adopt v4. The changes are concentrated in sections that reference `schema_version`, `IndexEntry v2`, or the sidecar terminology defined in §1.5. The catalog's relationship graph persistence and querying capabilities are new scope that should be added during metadexer's Phase 2 or early Phase 3 development, aligned with the product vision's sequencing.

Sections requiring updates (non-exhaustive):

- §1.5 (Terminology): Update IndexEntry definition to v4. Add `relationships` term.
- §2.2 (Relationship to shruggie-indexer): Update version references.
- §3.1 (Component Map): Update diagram annotation.
- §4 (The IndexEntry Contract): All subsections update from v2 to v4. Schema evolution rules remain unchanged in structure; only the version number and the description of additive fields need revision.
- §6.4.2 (CatalogIngestor): Update `schema_version` validation from `2` to `4`.
- §6.8 (Catalog Database Schema): Add relationship projection table or document how relationship data is indexed from `raw_entry`.

### 10.3. Technical specification (shruggie-indexer)
...
The following is a high-level ordering, not a sprint plan. Sprint-level decomposition will be done separately. Per the product vision, the v4 schema is a Phase 1 (Current Quarter) deliverable. All phases below fall within that scope.
...
Update the shruggie-indexer technical specification, changelog, and README to reflect the v4 architecture, new suffix convention, and legacy cleanup feature. Coordinate with the metadexer specification update (Section 10.2) to ensure the IndexEntry contract documentation is consistent across both repositories.

---

## 12. Future Work (Out of Scope for This Pivot)
...
- **Catalog relationship graph** (persistence, querying, visualization, bidirectional discovery). Ownership: metadexer catalog module.
- **Cross-session relationship analysis** (detecting sidecar additions/removals between indexing sessions). Ownership: metadexer catalog module.
...
- **Extraction pipeline integration** (using catalog-persisted relationship graphs as input for entity extraction and cross-document correlation). Ownership: metadexer extraction primitives (product vision Phase 2).
...
**4. Bidirectional relationship annotation:** Relationships are directional only. The sidecar-like file's entry points at the parent-like file. The parent receives no back-reference. Bidirectional discovery, graph inversion, and consolidated asset views are catalog responsibilities. See Section 10.1.
...
|------|----------|--------|
| 2026-03-21 | `20260321-005` | Initial draft of the sidecar architecture pivot. |
| 2026-03-24 | `20260321-005` | Ratified. Resolved six design decisions (Section 13). |
| 2026-04-01 | `20260401-001` | New edition (this document). Supersedes `20260321-005`. Added Section 9 (Strategic Context) incorporating the ShruggieTech Unified Product Vision and metadexer spec. Expanded ecosystem impact analysis (Section 10) with metadexer specification update scope. Updated all "shruggie-catalog" references to "metadexer catalog module." Added extraction pipeline integration to Section 12 (Future Work). No changes to ratified architectural decisions, schema design, or implementation sequencing. |
