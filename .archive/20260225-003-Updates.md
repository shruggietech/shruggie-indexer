# shruggie-indexer — Pending Updates (2026-02-24, Batch 2)
...
4. **Regenerate the HTML and PDF renderings** of the spec after all textual updates are complete, using the same VS Code plugin workflow that produced the current `.html` and `.pdf` files. The regenerated files go to `.archive/` with the correct prefix per the naming convention established in Section 4.
