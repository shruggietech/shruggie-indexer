...
### 3.3. ExifTool Error on Unsupported File Types

**Problem:** Processing a `.7z` archive file via the GUI produces the following warning:
...
ExifTool returns exit status 1 for file types it cannot parse. This is expected behavior for archive formats like `.7z`, `.zip`, `.tar`, etc. The current code treats this as an error condition and logs a WARNING, which is misleading -- it implies something is broken rather than simply unsupported.

**Required behavior:**

1. ExifTool exit status 1 for unrecognized file types should be handled gracefully. Log at `DEBUG` level (not WARNING) with a message like: `"ExifTool: no metadata extracted for <filename> (unsupported format)"`.
2. Review the `exiftool.exclude_extensions` default list and consider adding common archive formats (`.7z`, `.zip`, `.tar`, `.gz`, `.bz2`, `.xz`, `.rar`, `.zst`) to avoid unnecessary exiftool invocations entirely.
3. The pyexiftool "resetting" behavior (restarting the persistent exiftool process after a non-zero exit) should be evaluated -- a non-zero exit for an unsupported file type should not require a process reset if the exiftool process is still healthy.

**Affected files:**

| File | Role |
|------|------|
| `src/shruggie_indexer/core/exif.py` | Error handling for exiftool exit codes, extension exclusion list |
| `src/shruggie_indexer/config/defaults.py` | Default extension exclusion list |
| `docs/user-guide/configuration.md` | Document updated exclusion list |

**Spec reference:** `shruggie-indexer-spec.md` SS6.6 -- EXIF and Embedded Metadata Extraction.

**Acceptance criteria:**

- Processing a `.7z` file does not produce a WARNING-level log message.
- A DEBUG-level message indicates the file type is unsupported.
- Common archive formats are excluded by default.
- The exiftool persistent process is not reset unnecessarily.
...
| SS6.6 -- EXIF and Embedded Metadata Extraction | Section 3, item 3.3 / Section 4 | Document graceful handling of unsupported file types, updated default extension exclusions, and user-configurable key exclusion set. |
